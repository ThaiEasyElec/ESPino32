...
#if CONFIG_OV2640_SUPPORT
        case OV2640_PID:
		   SCCB_Write(slv_addr,0xFF,0x01);
            SCCB_Write(slv_addr,0x12,0x80);
...
esp_err_t camera_init(const camera_config_t* config)
{
	...
	//STK
	s_state->sensor.set_hmirror(&s_state->sensor,config->mirror);
	s_state->sensor.set_vflip(&s_state->sensor,config->vflip);
	s_state->sensor.set_whitebal(&s_state->sensor,config->whitebal);
    ...
    ESP_LOGE(TAG, "Test pattern enabled");
#endif
    ...
    if (pix_format == PIXFORMAT_GRAYSCALE) {
        ...
        s_state->bytes_per_pixel = 2;       // camera sends YUYV
    ...
    ESP_LOGE(TAG, "bpp: %d, fb_size: %d, mode: %d, width: %d height: %d",
            s_state->bytes_per_pixel, s_state->fb_size, s_state->sampling_mode,
    ...
    ESP_LOGE(TAG, "Initializing I2S and DMA");
    i2s_init();
    err = dma_desc_init();
    if (err != ESP_OK) {
...
static esp_err_t dma_desc_init()
{
    ...
    size_t line_size = s_state->width * s_state->bytes_per_pixel *
...
static size_t get_fb_pos()
{
    return s_state->dma_filtered_count * s_state->width *
            s_state->bytes_per_pixel / s_state->dma_per_line;
}
...
{
//    uint8_t* pfb = s_fb + s_line_count * s_buf_line_width / 2;
//    // Get pointer to the current DMA buffer
//    const uint32_t* buf = s_dma_buf[buf_idx];
//    for (int i = 0; i < s_buf_line_width / 8; ++i) {
//        pfb[0] = (buf[0] & 0xff0000) >> 16;
//        pfb[1] = (buf[2] & 0xff0000) >> 16;
//        pfb[2] = (buf[4] & 0xff0000) >> 16;
//        pfb[3] = (buf[6] & 0xff0000) >> 16;
//        buf += 8;
//        pfb += 4;
//    }
}
...
void DebugEnable(unsigned char en)
{
